...
export async function seedOfflineNote(userId: string, title = '', content = ''): Promise<Note> {
  ...
  await getOfflineDb(userId).notes.add({ id, userId, title, content, pinned: false, deletedAt: null,
    ...
    // LocalNote declares these nullable, not optional. src/test is outside
    // tsconfig.app.json, so omitting them would drift silently.
    encryptedPayload: null, encryptionIv: null, encryptionVersion: null, contentHash: null });

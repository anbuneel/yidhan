...
10. **Light theme depth, not more UI** — The light version should keep its softness but needs a clearer focal plane. Darker shadow pocket behind the manuscript surface (warm terracotta-tinted `rgba(120, 80, 60, ...)` instead of grey). Stronger tonal separation between the landing page background and the manuscript so it reads as a distinct object. The manuscript itself should match the real editor's surface treatment (per P0 #4) — depth comes from the *surroundings*, not from making the manuscript prettier than the product. The light mode should feel sunlit and tactile, not pale. *(Test in browser: exact shadow opacity and background tint need visual tuning — these are feel decisions, not spec decisions.)*
